...
2. **On each Bash/shell command** — the agent's shell tool is gated too (`Bash` for claude-code/codex, `bash` for pi/opencode). Commands that would let the agent free itself — `ironlint trust`, or a Bash write to `.ironlint.yml` / `.ironlint/gates/` — are denied via `ironlint gate-bash`, a separate built-in that is not a `check` and not trust-gated (it fires even with no `.ironlint.yml`). See the per-adapter "Bash gate" sections and `docs/superpowers/specs/2026-07-06-bash-gate-self-trust-prevention-design.md`.
