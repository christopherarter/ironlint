...
- **`ironlint-core`** — library. Modules:
  ...
  - `arch` — architecture enforcement (`architecture:` block): tree-sitter import extraction, the `DepGraph` + `evaluate`, and `lowering` to the synthetic `__arch__` check that shells out to `ironlint arch`. The subprocess does the work; the runner stays pure.
...
- **`ironlint-cli`** — thin binary, name `ironlint`. `cli.rs` defines clap subcommands; `commands/{check,validate,init,explain,show_resolved_config,doctor,trust,arch,gate_bash,update,schema,watch,sweep,config,error_report}.rs` are one-function adapters into core (`init` is a module dir; `config`/`error_report`/`sweep` are shared helpers).
...
**The check ABI** (locked stability surface — every adapter must satisfy it, every check `run` may rely on it): `$IRONLINT_FILE` (absolute path of the single file under check; not set for `pre-commit`), `$IRONLINT_FILES` (newline-joined list of all files under check; single entry for `write`, all staged files for `pre-commit`), `$IRONLINT_ROOT` (project root = the check's cwd), `$IRONLINT_EVENT` (`write`/`pre-commit`), `$IRONLINT_TMPFILE` (write-only, and only when the check's `run`/`steps` reference it: absolute path to an ironlint-materialized temp file — sibling of `$IRONLINT_FILE`, same extension — holding the proposed content; auto-removed after the check), `$IRONLINT_BIN` (absolute path to the `ironlint` binary itself, so the lowered `__arch__` check invokes the same binary rather than resolving `ironlint` from `PATH`; falls back to the bare name `ironlint` if the executable path cannot be determined), `$IRONLINT_PROPOSED_MANIFEST` (optional; absolute path to a manifest of sibling proposed files — tab-separated `file_path\tcontent_path` lines — so the lowered `__arch__` check can see cross-file imports within a single atomic patch; absent when no manifest is provided, preserving backward-compatible behavior), the proposed post-edit content on **stdin** (empty for `pre-commit`). No string templating — the path travels only as an env value, never spliced into `run`.
...
- An `architecture:` block declares named layers (each a list of `globs`, standard `globset` full-path semantics) and `rules` (each `from: <layer>` + `may_import: [<layer>...]`; an empty list forbids all imports out of that layer). It lowers to a synthetic `__arch__` check — don't define a check named `__arch__` yourself. Example:
  ```yaml
  architecture:
    layers:
      - name: presentation
    ...
    rules:
      - from: presentation
        may_import: []   # presentation may not import anything
  ```
