...
pub fn lower_architecture(cfg: &mut Config) -> Result<()> {
    ...
    let run = "\"$IRONLINT_BIN\" arch check --layers \"$IRONLINT_ARCH_LAYERS\" --root \"$IRONLINT_ROOT\" ${IRONLINT_EVENT:+--event \"$IRONLINT_EVENT\"} ${IRONLINT_FILE:+--file \"$IRONLINT_FILE\"} ${IRONLINT_PROPOSED_MANIFEST:+--proposed-manifest \"$IRONLINT_PROPOSED_MANIFEST\"}".to_string();
    cfg.checks.insert(
        "__arch__".to_string(),
        Check {
            files: vec!["**/*".to_string()],
