...
- Mutation testing is a **local, ad-hoc** investigative tool — not a CI gate (would burn runner minutes). Run `cargo install cargo-mutants` once, then point it at a file or diff: `cargo mutants --file 'crates/hector-core/src/<name>.rs'` for a single file, or `git diff main.. > pr.diff && cargo mutants --in-diff pr.diff` for the PR's worth of changes. A surviving mutant is a place where the tests executed the code but didn't verify what it does — treat survivors in code you touched as a coverage gap.
